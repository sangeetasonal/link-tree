...
const [enabledLinks, setEnabledLinks] = useState(
  ...
  const handleToggleSwitch = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/auth/add-link", {
        title: linkTitle,
        url: linkUrl,
  ...
  const fetchProfileImage = async () => {
    try {
      ...
      const response = await axios.get("http://localhost:5000/api/auth/profile-image", {
        headers: {
  ...
  const handleUpdateUser  = async () => {
    ...
    try {
      const response = await axios.patch("http://localhost:5000/api/auth/update-user", {
        ...userInfo,
...
const setActiveTab = (tab) => {
  ...
  const handleSave = async () => {
    try {
      ...
      // Update nickname
      const nicknameResponse = await axios.patch("http://localhost:5000/api/auth/update-nickname", {
      ...
      // Update bio
      const bioResponse = await axios.patch("http://localhost:5000/api/auth/update-bio", {
        bio: bio,
      }, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      // Update banner color
      await axios.patch("http://localhost:5000/api/auth/update-banner", {
  ...
  const handleDeleteLink = async (linkId) => {
    try {
      const response = await axios.delete(`http://localhost:5000/api/auth/links/${linkId}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
  ...
  const generateUniqueLink = () => {
    ...
    const uniqueLink = `http://localhost:5173/preview/${formattedNickname}`; // Adjust the port if necessary
  ...
  const saveSettings = async () => {
    try {
      // Save layout
      await axios.post("http://localhost:5000/api/auth/save-layout", {
      ...
      // Save font settings
      await axios.patch("http://localhost:5000/api/auth/save-font", {
      ...
      await axios.patch("http://localhost:5000/api/auth/save-button-color", {
 ...
 // Save theme color
 await axios.patch("http://localhost:5000/api/auth/save-theme-color", {
  ...
  headers: {
    ...
    // Save button styles
    await axios.patch("http://localhost:5000/api/auth/save-button-styles", {
